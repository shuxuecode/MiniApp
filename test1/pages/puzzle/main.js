...
function move(obj, id) {
  ...
  if (target_d != 0) {
    ...
    // console.log(d_posXY[target_d][0]);
    // console.log(d_posXY[target_d][1]);


    var aaa = d_posXY[target_d][0];
    var bbb = d_posXY[target_d][1];
  ...
  if (finish_flag == true) {
    ...
    wx.showToast({
      title: '恭喜你！用时' + obj.data.timerInnerHTML,
      icon: 'success',
      duration: 2000
    })
  }
...
function random_d(obj) {
  for (var i = 9; i > 1; --i) {
    var to = parseInt(Math.random() * (i - 1) + 1);//产生随机数，范围为1到i，不能超出范围，因为没这个id的DIV
    if (d[i] != 0) {

      var mmm = d_posXY[to][0];
      var nnn = d_posXY[to][1];

      switch (d[i]) {
        case 1:
          obj.setData({ d1StyleLeft: mmm, d1StyleTop: nnn });
          break;
        case 2:
          obj.setData({ d2StyleLeft: mmm, d2StyleTop: nnn });
          break;
        case 3:
          obj.setData({ d3StyleLeft: mmm, d3StyleTop: nnn });
          break;
        case 4:
          obj.setData({ d4StyleLeft: mmm, d4StyleTop: nnn });
          break;
        case 5:
          obj.setData({ d5StyleLeft: mmm, d5StyleTop: nnn });
          break;
        case 6:
          obj.setData({ d6StyleLeft: mmm, d6StyleTop: nnn });
          break;
        case 7:
          obj.setData({ d7StyleLeft: mmm, d7StyleTop: nnn });
          break;
        case 8:
          obj.setData({ d8StyleLeft: mmm, d8StyleTop: nnn });
          break;
        default:
          break;
  ...
  onLoad: function (options) {
    ...
    wx.getSystemInfo({
      success: function (res) {
        ...
        that.setData({
          d2StyleLeft: cellWidth,
          d2PositionX: cellWidth,
          //
          d3StyleLeft: cellWidth * 2,
          d3PositionX: cellWidth * 2,
          //
          d4StyleTop: cellWidth + 5,
          d4PositionY: cellWidth + 5,

          //
          d5StyleLeft: cellWidth,
          d5StyleTop: cellWidth + 5,
          d5PositionX: cellWidth,
          d5PositionY: cellWidth + 5,
          //
          d6StyleLeft: cellWidth * 2,
          d6StyleTop: cellWidth + 5,
          d6PositionX: cellWidth * 2,
          d6PositionY: cellWidth + 5,
          //
          d7StyleTop: cellWidth * 2 + 10,
          d7PositionY: cellWidth * 2 + 10,
          //
          d8StyleLeft: cellWidth,
          d8StyleTop: cellWidth * 2 + 10,
          d8PositionX: cellWidth,
          d8PositionY: cellWidth * 2 + 10,
          ///
          viewHeight: cellWidth,
          fontSize: cellWidth,
          lineHeight: cellWidth,
          //
          containerHeight: cellWidth * 3 + 10
    ...
    d_posXY = new Array(
      ...
      [cellWidth * 2, cellWidth + 5],
      [0, cellWidth * 2 + 10],
      [cellWidth, cellWidth * 2 + 10],
      [cellWidth * 2, cellWidth * 2 + 10]
    );
